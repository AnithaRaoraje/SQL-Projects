CREATE DATABASE IF NOT EXISTS walmartsalesdb;
...
# -----------------------------------------------------------------------------------------------
# Selecting all the columns from the sales table
SELECT 
    *
FROM
    sales;
...
# Creating a time of day column and adding it to the table
SELECT 
    time,
    (CASE
...
FROM
    sales;
# -----------------------------------------------------------------------------------------------
# Creating a Day Name column and adding it to the table
...
FROM
    sales;
# -----------------------------------------------------------------------------------------------
# Creating a Month Name column and adding it to the table
...
SELECT 
    payment, COUNT(payment) AS common_payment_method
FROM
    sales
GROUP BY payment
...
SELECT 
    month_name, SUM(ROUND(total, 2)) AS total_revenue
FROM
    SALES
GROUP BY month_name
...
SELECT 
    month_name, SUM(ROUND(cogs, 2)) AS total_cogs
FROM
    sales
GROUP BY month_name
...
SELECT 
    product_line, SUM(ROUND(total, 2)) AS total_revenue
FROM
    sales
GROUP BY product_line
ORDER BY total_revenue DESC
...
SELECT 
    city, SUM(ROUND(total, 2)) AS total_revenue
FROM
    sales
GROUP BY city
ORDER BY total_revenue DESC
...
SELECT 
    product_line, SUM(ROUND(vat)) AS VAT
FROM
    sales
GROUP BY product_line
ORDER BY VAT DESC
...
SELECT 
    day_name, time_of_day, COUNT(invoice_id) AS total_sales
FROM
    sales
...
FROM
    sales
WHERE
    day_name NOT IN ('Saturday' , 'Sunday')
GROUP BY day_name , time_of_day;
# -----------------------------------------------------------------------------------------------
# 2.Identify the customer type that generates the highest revenue.
SELECT 
    customer_type, SUM(ROUND(total, 2)) AS total_sales
FROM
    sales
...
SELECT 
    city, SUM(ROUND(VAT)) AS total_VAT
FROM
    sales
GROUP BY city
...
SELECT 
    customer_type, SUM(ROUND(VAT)) AS total_VAT
FROM
    sales
GROUP BY customer_type
ORDER BY total_VAT DESC
...
SELECT 
    customer_type, COUNT(*) AS most_buyer
FROM
    sales
GROUP BY customer_type
...
SELECT 
    branch, gender, COUNT(gender) AS gender_distribution
FROM
    sales
...
SELECT 
    branch, time_of_day, AVG(rating) AS average_rating
FROM
    sales
...
SELECT 
    branch, day_name, AVG(rating) AS average_rating
FROM
    sales
